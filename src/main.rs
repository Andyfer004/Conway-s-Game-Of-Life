...
fn initialize_pattern(framebuffer: &mut Framebuffer) {
    ...
    for i in (0..WIDTH).step_by(20) {
        for j in (0..HEIGHT).step_by(20) {
            add_pattern(framebuffer, &block, i, j, COLOR_STILL_LIFE);
            add_pattern(framebuffer, &beehive, i + 5, j, COLOR_STILL_LIFE);
            add_pattern(framebuffer, &loaf, i + 10, j, COLOR_STILL_LIFE);
            add_pattern(framebuffer, &boat, i + 15, j, COLOR_STILL_LIFE);
            add_pattern(framebuffer, &tub, i, j + 5, COLOR_STILL_LIFE);
            add_pattern(framebuffer, &blinker, i + 5, j + 5, COLOR_OSCILLATOR);
            add_pattern(framebuffer, &toad, i + 10, j + 5, COLOR_OSCILLATOR);
            add_pattern(framebuffer, &beacon, i + 15, j + 5, COLOR_OSCILLATOR);
            add_pattern(framebuffer, &pulsar, i, j + 10, COLOR_OSCILLATOR);
            add_pattern(framebuffer, &pentadecathlon, i + 5, j + 10, COLOR_OSCILLATOR);
            add_pattern(framebuffer, &glider, i + 10, j + 10, COLOR_SPACESHIP);
            add_pattern(framebuffer, &lwss, i + 15, j + 10, COLOR_SPACESHIP);
            add_pattern(framebuffer, &mwss, i, j + 15, COLOR_SPACESHIP);
            add_pattern(framebuffer, &hwss, i + 5, j + 15, COLOR_SPACESHIP);
        }
    }
}
